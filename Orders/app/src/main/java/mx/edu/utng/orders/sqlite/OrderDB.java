...
public class OrderDB extends SQLiteOpenHelper {
    ...
    interface References{
        String ID_ORDER_HEADER = String.format("REFERENCES %s(%s)"+ "ON DELETE CASCADE", Tables.ORDER_HEADER, OrderContract.OrderHeaders.ID);
        String ID_PRODUCT = String.format("REFERENCES %s(%s)"+ "ON DELETE CASCADE", Tables.PRODUCT, OrderContract.Products.ID);
        String ID_CUSTOMER = String.format("REFERENCES %s(%s)"+ "ON DELETE CASCADE", Tables.CUSTOMER, OrderContract.Customers.ID);
        String ID_METHOD_PAYMENT = String.format("REFERENCES %s(%s)"+ "ON DELETE CASCADE", Tables.METHOD_PAYMENT, OrderContract.MethodsPayment.ID);
    }
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(String.format(
                ...
                Tables.ORDER_DETAIL, BaseColumns._ID,
        ...
        db.execSQL(String.format("CREATE TABLE %s (%s INTEGER PRIMARY KEY" +
                        " AUTOINCREMENT, %s TEXT NOT NULL UNIQUE, %s TEXT NOT NULL," +
                        " %s INTEGER NOT NULL, %s INTEGER NOT NULL CHECK(%s>=0))",
                Tables.CATEGORY, BaseColumns._ID,
                OrderContract.Categories.ID,
                OrderContract.Categories.NAME,
                OrderContract.Categories.DESCRIPTION,
                OrderContract.Categories.NUMBER,
                OrderContract.Categories.NUMBER));
    }
